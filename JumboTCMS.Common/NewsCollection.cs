    ...
    public class NewsCollection
    {
        ...
        public string GetThumtnail(string mainSiteUrl, bool mainSite, string RemoteFileUrl, string SavePath, bool isSave, int width, int height)
        {
            ...
            if (_LocalFileUrl == RemoteFileUrl && !_LocalFileUrl.EndsWith("_thumbs.jpg"))//表示非远程图片，非缩略图
            {
                ...
                _LocalFileUrl = SavePath + FolderName + "/" + filename;
                JumboTCMS.Utils.ImageHelp.LocalImage2Thumbs(HttpContext.Current.Server.MapPath(RemoteFileUrl), HttpContext.Current.Server.MapPath(_LocalFileUrl), width, height, "Fill");
        ...
        public bool SaveRemotePhoto(string fileName, string RemoteFileUrl, int width, int height)
        {
            ...
            if (width > 0 && height > 0)
                JumboTCMS.Utils.ImageHelp.Image2Thumbs(getImage, fileName, width, height, "Fill");
            else
            {
                try
                {
                ...
                catch
                {
                    ...
                    t.Save(fileName, JumboTCMS.Utils.ImageHelp.ImgFormat(fileName));
                    t.Dispose();
                }
            }
            getImage.Dispose();
