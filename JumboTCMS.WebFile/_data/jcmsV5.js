...
site.Name = '将博CMS通用版';
site.Name2 = '将博';
site.Url = 'http://www.jumbotcms.net';
site.Dir = '/';
site.CookieDomain = '.jumbotcms.net';
site.CookiePrev = 'jcmsV5';
site.AllowReg = true;
site.CheckReg = false;
...
{recordcount: 5, table: [{no: 0, id: 1, title: '文章', type: 'article'},{no: 1, id: 2, title: '图片', type: 'photo'},{no: 2, id: 3, title: '下载', type: 'soft'},{no: 3, id: 4, title: '视频', type: 'video'},{no: 4, id: 5, title: '文档', type: 'document'}]}
...
{recordcount: 5, table: [{title: 'QQ账号', code: 'tencent',enabled: 0},{title: '新浪微博', code: 'sina',enabled: 0},{title: '开心网账号', code: 'kaixin',enabled: 0},{title: '百度账号', code: 'baidu',enabled: 0},{title: '人人网账号', code: 'renren',enabled: 0}]}
...
function _jcms_GetOAuthBar(){
    ...
    for(var i=0;i<_oauths.length;i++){
        if(_oauths[i].enabled==1)
            _oauthbar += '<li><a href="'+site.Dir+'app/'+_oauths[i].code+'/oauth.aspx?type=login"><img src="'+site.Dir+'statics/passport/'+_oauths[i].code+'16.gif" title="使用'+_oauths[i].title+'登录"  alt="使用'+_oauths[i].title+'登录" /></a></li>';
    }
    return _oauthbar;
}
//绑定链接
function _jcms_SetOAuthBar(){
    ...
    for(var i=0;i<_oauths.length;i++){
        if(_oauths[i].enabled==1){
            _enablednum++;
            $('#oauth_'+_oauths[i].code).attr("title","使用"+_oauths[i].title+"登录").attr("href",site.Dir+"app/"+_oauths[i].code+"/oauth.aspx?type=login");
        }
        else
...
function get_user_status(w)
{
    ...
    $.ajax({
        ...
        url:        _jcms_Host() + site.Dir + "ajax/user.aspx?oper=ajaxLoginbar&time="+(new Date().getTime()),
        success:    function(d){
            if(d.result!="1" && w==1)
                alert(d.returnval);
            else
            ...
            $("#user_status").show();
            ajaxShowWeather('search_bar_weather');//显示天气(2011.03.06)
        }
    });
}
// 显示当前日期，时间
...
function ajaxViewCount(cType,id,randomid,needadd)
{
    ...
    $.ajax({
        ...
        url:        _jcms_Host() + site.Dir + "ajax/content.aspx",
        error:        function(XmlHttpRequest,textStatus, errorThrown){if(XmlHttpRequest.responseText!=""){alert(XmlHttpRequest.responseText);}},
        success:    function(d){
...
function ShowEditDiv(channeltype,channelid,contentid){
    ...
    if(masterid != ""){
        top.JumboTCMS.Popup.show(site.Dir+'modules/'+channeltype+'_admin_edit.aspx?ccid='+channelid+'&id='+contentid,-1,-1,true);
    }
}

function ajaxDiggAdd(cType,id)
{
    $.ajax({
        type:        "get",
        dataType:    "json",
        data:        "oper=ajaxDiggAdd&id="+id+"&cType="+cType+"&time="+(new Date().getTime()),
        url:        _jcms_Host() + site.Dir + "digg/ajax.aspx",
        error:        function(XmlHttpRequest,textStatus, errorThrown){if(XmlHttpRequest.responseText!=""){alert(XmlHttpRequest.responseText);}},
        success:    function(d){
            $("#ajaxDigg_"+id).text(d.count);
            $("#DiggSpan"+id).html("<a style='color:#cccccc;text-decoration:none;'>顶一下</a>");
