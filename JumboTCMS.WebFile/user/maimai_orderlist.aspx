...
<div id="wrap">
  <div id="main">
	...
	<tr class="noborder">
		<td align="left">
			<div style="float:left;margin-left:15px" class="photo48"><a href="{$T.record.productlink}" target="_blank" title="{$T.record.productname}"><img src="{$T.record.productimg}" onerror="this.src=site.Dir+'style/common/nophoto.jpg'" alt="{$T.record.productname}" width="48" height="48" /></a></div>
	...
	<tr>
		<td colspan="3" width="*"><div id="ajaxGetGoods_{$T.record.ordernum}"><img src='../style/loading.gif' /></div><script type="text/javascript">ajaxGetGoodsList('{$T.record.ordernum}');</script></td>
		...
		<td align="center">
			...
			<br />
			{#if $T.record.state == "0"}
			<a href="javascript:void(0);" onclick="JumboTCMS.Confirm('确定要支付该订单吗?', 'ajaxPay(\'{$T.record.ordernum}\')');">直接支付</a>
			{#else}
			<font color='#cccccc'>直接支付</font>
			{#/if}
			<br />
			{#if $T.record.state == "2"}
