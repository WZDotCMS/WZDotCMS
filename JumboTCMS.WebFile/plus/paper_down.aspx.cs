...
namespace JumboTCMS.WebFile.Modules.Paper.Plus
{
    public partial class _down : JumboTCMS.UI.FrontHtml
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            object[] _obj = doh.GetFields("jcms_module_paper", "PaperUrl,Points,Title");
            string paperUrl = _obj[0].ToString();
            int _Points = Str2Int(_obj[1].ToString(), 0);
            string _PaperTitle = _obj[2].ToString();
            //如果地址不为空
            if (paperUrl != "")
            {
                if (_Points > 0)//说明是需要扣除博币的，那么肯定要判断当前用户博币够不够
                {
                    if (!CanDownFile(ChannelId, id, _Points, _PaperTitle))
                    {
                        Response.Redirect("~/errordown.aspx");
                        Response.End();
                    }
                }
                DownloadFile(paperUrl);
            }
            else
        ...
        /// <param name="_PaperId">内容ID</param>
        /// <param name="_Points">扣除博币</param>
        /// <param name="_PaperTitle">文库中文标题</param>
        /// <returns></returns>
        private bool CanDownFile(string _ChannelId, string _PaperId, int _Points, string _PaperTitle)
        {
            string _UserId = "0";
            string _UserName = "";
            if (JumboTCMS.Utils.Cookie.GetValue(site.CookiePrev + "user") != null)
            {
                ...
                if (!_IsVIP)//给用户扣除博币,VIP不扣除
                {
                    ...
                    //不是刷分就是恶意下载，做个统计留意一下
                    doh.ConditionExpress = "DownDegree<50 and ChannelId=" + _ChannelId + " and PaperId=" + _PaperId + " and UserId=" + _UserId;
                    if (doh.Exist("jcms_module_paper_downlogs"))//说明已经扣过
                    {
                        doh.Reset();
                        doh.ConditionExpress = "DownDegree<50 and ChannelId=" + _ChannelId + " and PaperId=" + _PaperId + " and UserId=" + _UserId;
                        doh.Add("jcms_module_paper_downlogs", "DownDegree");
                        return true;
                    }
                    doh.Reset();
                    ...
                    string _OperInfo1 = "下载文库:<a href=\"" + Go2View(1, false, _ChannelId, _PaperId, false) + "\" target=\"_blank\">" + _PaperTitle + "</a>，扣除了" + _Points + "个点";
                    ...
                    doh.AddFieldItem("PaperId", _PaperId);
                    ...
                    doh.Insert("jcms_module_paper_downlogs");
                }
                return true;
            }
            else
