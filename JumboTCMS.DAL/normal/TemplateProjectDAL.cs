    ...
    public class Normal_TemplateProjectDAL : Common
    {
        ...
        public void GetListJSON(int _thispage, int _pagesize, string _wherestr, ref string _jsonstr)
        {
            using (DbOperHandler _doh = new Common().Doh())
            {
                ...
                sqlStr = JumboTCMS.Utils.SqlHelp.GetSql("[Id],[StartIP2],[EndIP2],[ExpireDate],[Enabled]", "jcms_normal_templateproject", "Id", _pagesize, _thispage, "desc", _wherestr);
                _doh.Reset();
                _doh.SqlCmd = sqlStr;
        ...
        public JumboTCMS.Entity.Normal_TemplateProject GetEntity(string _id)
        {
            JumboTCMS.Entity.Normal_TemplateProject templateproject = new JumboTCMS.Entity.Normal_TemplateProject();
            using (DbOperHandler _doh = new Common().Doh())
            {
                _doh.Reset();
